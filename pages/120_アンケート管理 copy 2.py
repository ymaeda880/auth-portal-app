...
def get_answer_score(
    *,
    ...
    for option in question.options:
        ...
        if (
            option.value != value
            and str(option.value) != str(value)
        ):
            continue
    ...
    for index, question in enumerate(
        ...
        if question.question_type in CHOICE_TYPES:
            ...
            for value in answered_values:
                ...
                # 0|使用したことがない
                #
                # のように，選択肢の左側へ数値を指定した場合，
                # その保存値を数値として集計する．
                #
                # 回答データでは "1" や "0" のような
                # 文字列として保持される場合があるため，
                # floatへ変換可能な値は数値として扱う．
                # ------------------------------------------------
                try:
...
def build_excel_bytes(
    *,
    ...
    with pd.ExcelWriter(
        ...
        free_text_worksheet.freeze_panes = "A2"

        if free_text_worksheet.max_row >= 1:
...
def render_publication_panel(
    *,
    ...
    # 公開設定 更新完了メッセージ
    #
    # 公開設定の保存後にst.rerun()しているため，
    # session_stateに保存したメッセージを再実行後に表示する．
    # pop()を使用し，メッセージは1回だけ表示する．
    # ------------------------------------------------------------
    publication_message = st.session_state.pop(
        K_PUBLICATION_MESSAGE,
        None,
    )
    ...
    with action1:
        ...
        # 公開操作 確認文言
        #
        # 登録済みアンケートを初めて公開する場合と，
        # 公開済みアンケートの公開設定を更新する場合で，
        # 確認チェックと補足説明の文言を切り替える．
        # --------------------------------------------------------
        if current_status == STATUS_DRAFT:
            publish_confirm_label = (
                "公開期日を確認したので公開する"
            )
            publish_action_label = "公開"

        else:
            publish_confirm_label = (
                "変更する公開期日を確認した"
            )
            publish_action_label = "公開設定を更新"
        ...
        st.caption(
            f"※ チェックしないと「{publish_action_label}」が押せません．"
        )
        ...
        #
        # 回答期限を未来へ延長した場合は，
        # publish_survey() により再びSTATUS_RUNNINGへ戻る．
        # --------------------------------------------------------
...
def render_aggregation_panel(
    *,
    ...
    try:
        ...
        # 回答ファイルを読み込む
        #
        # load_all_survey_responses() には，
        # submittedだけでなくdraftが含まれる可能性がある．
        # --------------------------------------------------------
        all_responses = load_all_survey_responses(
            paths,
            survey_id=survey_id,
        )
        ...
        draft_count = count_draft_responses(
            paths.db_path,
            survey_id=survey_id,
        )
        ...
        draft_records = list_draft_response_records(
            paths.db_path,
            survey_id=survey_id,
        )
