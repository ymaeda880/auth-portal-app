# -*- coding: utf-8 -*-
# auth_portal_app/pages/00_トップ.py
...
# app imports
# ============================================================
from lib.config import APP_LABELS, USERS_FILE, APP_HELP, APP_CAPTIONS
...
# lib（説明UI）
# ============================================================
from lib.explanation.exp_portal_top import (
	   render_portal_top_page_intro,
    render_portal_top_help_expander,
)
...
def render_pais_header(
    ...
    logo_path = APP_ROOT / logo_relpath

    c1, c2 = st.columns([4, 12], vertical_alignment="center")

    with c1:
        if logo_path.is_file():
            st.image(str(logo_path), width=logo_px)
        else:
            st.write("🐤")
...
with st.sidebar:
    if user:
        ...
        st.caption(
            ...
            "ログアウトしないでブラウザーを閉じても問題ありません．"
            "その場合は，最初にログイン後12時間はログインが有効になっています．"
        )
    ...
    for row in _grouper(apps, columns):
        ...
        for c, app in zip(cols, row):
            with c:
                ...
                st.caption(APP_CAPTIONS.get(app, app))
                #st.caption(f"`{app}`")
