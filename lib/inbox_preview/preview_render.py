# -*- coding: utf-8 -*-
# lib/inbox_preview/preview_render.py
...
# ------------------------------------------------------------
# PDF/Word/Excel preview helpers
# ------------------------------------------------------------
...
def render_preview(
    *,
    ...
    paths: Dict[str, Path],
    on_preview_open: Optional[Callable[[str, str], None]] = None,
) -> None:
    """
    selected:
      - item_id, kind, stored_rel, original_name などを含む dict
    resolve_file_path(stored_rel) -> Path
    paths:
      - pdf_preview, word_preview など（ページ側で mkdir 済み想定）
    on_preview_open(item_id, raw_kind):
      - preview 表示直前に呼ばれる（last_viewed 更新など）
    """
    item_id = str(selected["item_id"])
    raw_kind = str(selected.get("kind", "")).lower()
    stored_rel = str(selected.get("stored_rel") or "")
    path = resolve_file_path(stored_rel)
    ...
    # ✅ preview のみ「閲覧記録」を更新したい場合はここで呼ぶ
    if on_preview_open is not None:
        on_preview_open(item_id, raw_kind)
    ...
    if raw_kind == "excel":
        if path.suffix.lower() == ".xls":
            st.info(
                "このExcel形式（.xls）は現在プレビュー非対応です（保存・ダウンロードは可能）。\n"
                "必要なら .xlsx 形式に変換してからアップロードしてください。"
            )
            return
