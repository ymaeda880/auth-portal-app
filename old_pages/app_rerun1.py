...
from lib.config import APP_LABELS, USERS_FILE
...
# -------------------------------
# 共通: ログアウトして即座に未ログインUIへ切り替え
# -------------------------------
def logout_and_rerun(cm: stx.CookieManager):
    # Cookie 同期（初回レンダ直後のズレ回避）
    try:
        _ = cm.get_all()
    except Exception:
        pass

    # Cookie 削除（存在しないキーでの KeyError を吸収）
    try:
...
def launch(apps: list[str], title: str, need_user: bool = False, filter_fn=None, key_prefix: str = "go"):
    ...
    for i, app in enumerate(apps):
        ...
        if cols[i % len(cols)].button(label, use_container_width=True, key=f"{key_prefix}_{app}_{i}"):
            # クエリ文字列は付けない（JWTクッキーだけで本人識別＆権限判定）
...
with st.expander("🆕 サインアップ（任意）", expanded=False):
    ...
    apps_sel = st.multiselect("初期アクセス許可（apps）",
                              options=options, default=options[:1], key="signup_apps")
